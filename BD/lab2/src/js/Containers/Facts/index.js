...
class Facts extends Component {
  ...
  edit(e) {
    ...
    draw.push(
      <button key={itterationKey++} onClick={::this.save}>
        save
      </button>
    );
  ...
  delete(e) {
    ...
    const buff = facts_data.filter((item, index) => {
      if (index != e.target.id) return item;
    });
    ...
    fetch(
      `http://${local_ip}:3000/api/uploadFileData?requestValue=facts&data=${JSON.stringify(
        buff
      )}`,
      {
        method: "POST"
  ...
  save() {
    ...
    Object.values(facts_data).map(i => {
      ...
      Object.values(i).map((item, index) => {
        if (keys[index] === "id_plane") id_arr.push(item);
      });
    });
  ...
  render() {
    ...
    return (
      <div id="FactsContainer">
        ...
        <div className={"hide"}>
          <button onClick={::this.closeInputPanel}>x</button>
          {Object.values(facts_data).map((i, itter) => {
            buff = [];
            let keys = Object.keys(i);
