...
class Facts extends Component {
    ...
    takeData() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            fetch(`http://192.168.1.101:3000/api/generator?requestValue=${i}`, {
                method: 'POST',
            })
                .then(d => d.json())
                .then(d => {
    ...
    delete(e) {
        ...
        fetch(`http://192.168.1.101:3000/api/delete?requestValue=3`, {
            method: 'POST',
        })
            .catch((err) => console.log(err));

        fetch(`http://192.168.1.101:3000/api/uploadFileData?requestValue=facts&data=${JSON.stringify(buff)}`, {
            method: 'POST',
        })
    ...
    save() {
        ...
        if (flag) {
            ...
            fetch(`http://192.168.1.101:3000/api/uploadFileData?requestValue=facts&data=${JSON.stringify(facts_data)}`, {
                method: 'POST',
            })
                .then(() => console.log('save >>>>'))
                .catch((err) => console.log(err));
