...
class View extends Component {
    ...
    takeData(value) {
        ...
        fetch(`http://192.168.1.101:3000/api/generator?requestValue=${value}`, {
                method: 'POST',
            })
            .then(d => d.json())
            .then(d => {
    ...
    uploadFile(e) {
        ...
        fetch('http://192.168.1.101:3000/api/upload', {
            method: 'POST',
            body: formData,
        })
            .then(d => d.json())
    ...
    saveToDataBase() {
        ...
        // clear dataBase
        for (let i = 0; i < 3; i++) {
            fetch(`http://192.168.1.101:3000/api/delete?requestValue=${i}`, {
                method: 'POST',
            })
                .then(d => d.json())
                .then(d => {
        ...
        Object.values(data).map((item, index) => {
            fetch(`http://192.168.1.101:3000/api/uploadFileData?requestValue=${keys[index]}&data=${JSON.stringify(item)}`, {
                    method: 'POST',
                })
                    .then(d => d.json())
                    // .then(d => {
    ...
    delete(e) {
        ...
        if (flag) {
            ...
            fetch(`http://192.168.1.101:3000/api/delete?requestValue=${value}`, {
                method: 'POST',
            })
                .catch((err) => console.log(err));
