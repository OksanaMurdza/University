...
class Login extends Component {
    ...
    showChoice(model, name) {
        ...
        if (model.length > 0) {
            ...
            let buff = buffKey.map((item, i) => {
                let res = Object.values(item).map((item, index) => <li key={index}>{item}</li>);
                return (
                    <div key={i} className="MainList">
                        <button onClick={::this.deleteModel} className="delete" id={`${i}_${name}`}>Delete</button>
                        <p onClick={::this.edit} id={`${i}_${name}`} className={classname}>Redaction</p>
                        <ul>{res}</ul>
                        <hr/>
                    </div>
                )
    ...
    addInput() {
        ...
        let buff = buffKey[0].map((item, i) => {
            return <div key={i}> <label htmlFor={`input_${item}`}>{item}</label> <input type="text" className={`input_${item}`} ref={`input_${item}`}/></div>
        });
    ...
    save() {
        ...
        buffKey[0].map((item, i) => {
            saveData[`${item}`] = ReactDOM.findDOMNode(this.refs[`input_${item}`]).value;
            if (item === 'plane') {
                hack = 'plane'
                planeCode = this.state.plane.filter((i) => {
                    if (i.code === saveData[`${item}`])
                        return i
                });
            }
        ...
        if (this.state.save) {
            if (!planeCode.length && hack == 'plane') {
                ...
                model.push(saveData);
                choice ? this.setState({plane: model}) : this.setState({flight: model});
                choice ? ::this.showChoice(this.state.plane, 'plane') : ::this.showChoice(this.state.flight, 'flight');
            }
        } else {
                ...
                planeCode = this.state.plane.filter((i) => {
                    if (i.code === newModel[index].plane)
                        return i
                });
                if (!planeCode.length) {
                    console.error('please, add plane')
        ...
        return (
            <div className="wrapper">
               <div className="container">
                   <button onClick={::this.show} ref="showButton">Show</button>
                   <button onClick={() => ::this.add(true)}>Add</button>

                   <div className="hide" ref="showChoice">
                        <div className="choice">
                            <button onClick={::this.filterShow}>Filter</button>
                            <button onClick={() => ::this.showChoice(this.state.plane, 'plane')}>plane</button>
                            <button onClick={() => ::this.showChoice(this.state.flight, 'flight')}>flight</button>
                        </div>
                       {this.state.view}
                   </div>
                   <div className="hide" ref="add">
                       ...
                       {::this.addInput()}
                       <button onClick={::this.save}>{this.state.save ? 'Save' : 'edit'}</button>
                   </div>
