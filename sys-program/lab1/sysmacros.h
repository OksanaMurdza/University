...
#define emajor(x) \
  (major_t)(((unsigned int)(x) >> O_BITSMINOR) > O_MAXMAJ) ? NODEV : (((unsigned int)(x) >> O_BITSMINOR) & O_MAXMAJ)
 ...
 */
#define getemajor(x) (major_t)((((dev_t)(x) >> L_BITSMINOR) > L_MAXMAJ) ? NODEV : (((dev_t)(x) >> L_BITSMINOR) & L_MAXMAJ))
...
#define DEVCMPL(x)                                \
  (dev32_t)((((x) >> L_BITSMINOR) > L_MAXMAJ32 || \
             ((x)&L_MAXMIN) > L_MAXMIN32)         \
                ? NODEV32                         \
                : ((((x) >> L_BITSMINOR) << L_BITSMINOR32) | ((x)&L_MAXMIN32)))

#define DEVEXPL(x) \
  (((x) == NODEV32) ? NODEV : makedevice(((x) >> L_BITSMINOR32) & L_MAXMAJ32, (x)&L_MAXMIN32))
